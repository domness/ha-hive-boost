...
SERVICE_START_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional("duration", default=DEFAULT_DURATION): vol.In(DURATION_OPTIONS),
    }
)
...
class HiveBoostCoordinator:
        ...
        for entity_id, state in stored.items():
            ...
            if ends_at_str:
                ...
                if ends_at and ends_at > now:
                    # Boost was active before restart — restore it
    ...
    async def _handle_start_boost(self, call: ServiceCall) -> None:
        ...
        # Cancel any existing boost timer for this entity
        ...
        _LOGGER.info(
            "Boost started for %s: %.1f°C for %s (ends %s)",
            entity_id, temperature, duration, ends_at,
        )

        # Fire an event so dashboards / automations can react
        self.hass.bus.async_fire(
            f"{DOMAIN}_boost_started",
            {"entity_id": entity_id, "temperature": temperature, "duration": duration},
        )
